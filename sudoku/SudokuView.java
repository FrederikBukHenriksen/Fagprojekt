					...
					if (xGrid != -1) {
						if (sudokuboardCells.get(xGrid).get(yGrid).isSelected()) {
							if (actionEvent.getActionCommand()
								...
								// Check for what field is selected. Sets number to 0, if the same number was already selected
								sudokuboardCells.get(xGrid).get(yGrid).setText(null);
								sudoku[xGrid][yGrid] = 0;
							} else {
								sudokuboardCells.get(xGrid).get(yGrid).setText(actionEvent.getActionCommand());// check for if field is selected, sets cell to number
